...
//			return intersection;
		 ...
		 * 
		 */		
		public static function hitTestAnimation(x:Number,y:Number,animation:IAnimation,color:uint=0x50):Boolean
		{
			var obj:BitmapData=getTransBitmapData(animation.getAnimation(),new Matrix(animation.Zoom,0,0,animation.Zoom,0,0));
			var p:Point=(animation as IDisplayObject).Instance.parent.globalToLocal(new Point(x,y));
			return obj.hitTest(new Point(animation.OriginX-(animation.XOffset+animation.CurrTexture.frameX*animation.Zoom),animation.OriginY-(animation.YOffset+animation.CurrTexture.frameY*animation.Zoom)),color,p);
		}
		/**
		 * 画出一个对象的非透明区域 
		 * @param displayObject
